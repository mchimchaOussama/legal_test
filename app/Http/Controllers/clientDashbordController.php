...
class clientDashbordController extends Controller
{
    ...
    public function fetchInvoiceDetails(Request $request)
    {
        ...
        if ($paiement) {
            ...
            foreach ($details as $detail) {
                $html .= '<tr>';
                $html .= '<td>' . ($detail->thematique->theme ?? 'N/A') . '</td>'; // Replace with actual column if different
    ...
    public function fetchleads(Request $request)
    {
        ...
        foreach ($details as $detail) {
            ...
            $html .= '<tr>';
            $html .= '<td>' . ($detail->thematique->theme ?? 'N/A') . '</td>';
    ...
    public function dashbord_get()
    {
        ...
        // Get thematiques and departements
        $data = $this->getThematicsData();
        $thematiques = $data['thematiques'];
        $departements = $data['departements'];

        // Total number of leads for the client
        $statLead = Payment::where('client_id', $clientId)->count();
        // Calculate the lead count for each invoice
        ...
        return view('marketplace.dashbord.dashbord', compact('thematiques', 'departements', 'invoices', 'nbrFact', 'statLead','invoicesStat','totalPayments','leads','Commande'));
    }
...
public function sendVerificationCode(Request $request)
{
    ...
    public function demandez_devis_view()
    {
        ...
        return view("Marketplace.demandes",compact('thematiquesStatistiques'));
