...
<body class="body">
    ...
    <div id="wrapper">
                ...
                <section class="h-100">
                    <div class="container h-100 py-5">
                        <section class="h-100 gradient-custom">
                                ...
                                <div class="col-md-5">
                                  <div class="card mb-4">
                                    <div class="card-header py-1">
                                        @if (session('cart') && count(session('cart')) > 0)
                                      <h6 class="mb-0">Panier - {{count(session('cart'))}} leads</h6>
                                      @else
                                      <h6 class="mb-0">Panier Vide</h6>
                                      @endif
                                    </div>
                                    <div class="card-body">
                                      <!-- Single item -->
                                          ...
                                          <div class="row d-flex justify-content-between align-items-center">
                                            ...
                                            <div class="col-md-1  text-end">
                                              <form action="{{ route('cart.update') }}" method="POST" id="cart-form">
                                                @csrf
                                              <input class="form-check-input" onclick="showAlert(this)" value="{{$details['id']}}" type="checkbox" name="check"  {{ $details['checked'] ? 'checked' : '' }} style="scale: 1.3;">
                                            
                                              <input type="hidden" id="elemcheck" name="listitem" />
                                              </form></div>
                                            <div class="col-md-1 text-end">
                                                <form action="{{ route('cart_remove',$details['id']) }}" method="POST">
                                  ...
                                  <div class="card mb-4">
                                    <div class="card-body">
                                      <form method="post" action="{{route('coupon.apply')}}">
                                        <h7>Vous avez un Coupon ?</h7>
                                        <div class="input-group">
                                         
                                            @csrf
                                            <input type="text" class="form-control" placeholder="Entrez votre code" name="coupon" aria-label="Code de coupon">
                                            <button class="btn btn-primary btn-sm" type="submit">Appliquer</button>
                                      
                                           </div>
                                        <span class="text-muted mt-2">Entrez votre code de réduction pour bénéficier d'une remise.</span>
                                      </form>  
                                    </div>
                                  </div>
                                  <div class="card mb-4 mb-lg-0">
                                    <div class="card-body">
                                      <p><strong>Nous acceptons les modes de paiements suivants: </strong></p>
                              
                                        <i class="fab fa-cc-paypal fa-3x" title="PayPal"></i>
                                        <i class="fab fa-cc-stripe fa-3x" title="stripe"></i>
                          ...
                          <form method="POST" action="{{route('paiement.show')}}">
                            <input type="hidden" name="price" value="{{$total}}"/>
                                  @csrf    <button  type="submit" data-mdb-button-init data-mdb-ripple-init class="btn btn-primary  ">
                                        Passer au Paiement
                                      </button>
                          </form>
                                    </div>
       ...
       //   alert("test");
          if(element.checked){
            elemcheck+=","+element.value;
           // alert("cocheé");
            
          }
          
        });
       elemcheck= elemcheck.substr(1);

   
          document.getElementById('elemcheck').value=elemcheck;
